...
{
        int w_time[n], t_at[n], total w_time = 0, total_t_at = 0;
        ...
        for(int i=0 ;i<n ;i++)
        {
                total w_time = total w_time + w_time[i];
		total t_at = total t_at + t_at[i];
		printf(" %d ",(i+1));
		printf("\t%d ", b_time[i]);
		printf("\t%d",  w_time[i]);
		printf("\t%d\n", t_at[i]);
	}
	        int s=(float)total w_time / (float)n;
	        int t=(float)total t_at / (float)n;
...
int main()
{
        ...
        printf("Processes is ", size of processes[0])
